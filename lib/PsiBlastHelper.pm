...
Chunks get short name + different number for each chunk (+ sufix '_large' if larger than --fasta_size or in top N sequences by size). This is because BLAST works really slowly for large sequences and they are processed separately one by one.
So you need to provide input file, size of the chunk, chunk name and either top N or length of sequences to run separately. 
You also meed to provide --cpu or --cpu_l to split SGE or HTCondor script on this number of jobs. The idea here is to reduce a number of BLAST database copies, which can lead to failed jobs if out of disk space on specific node. This means that one script == one database copy and multiple BLAST processes.
...
Martin Sebastijan Šestak E<lt>martin.s.sestak@gmail.comE<gt>
